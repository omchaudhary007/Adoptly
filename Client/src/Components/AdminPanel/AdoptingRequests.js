...
const AdoptingRequests = () => {
  ...
  const fetchForms = async () => {
    try {
      const response = await fetch('http://localhost:999/form/getForms');
      if (!response.ok) {
  ...
  const fetchPets = async () => {
    try {
      const response = await fetch('http://localhost:999/approvedPets');
      if (!response.ok) {
        throw new Error('An error occurred');
      }
      const data = await response.json();
          ...
          <div className='popup-content'>
            <div className='pet-view-card'>
              <div className='pet-card-pic'>
                <img src={`http://localhost:999/images/${selectedPet.filename}`} alt={selectedPet.name} />
              </div>
